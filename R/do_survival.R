	...
	for (ra_type in ra_types) {
		...
		for (sensspec in sensspecs) {
			...
			for(chip_type in chip_types) {
				...
				str_form = glue('Surv(time = time_at_risk, event = event) ~ age_at_biosample + age2 + sex + race + ever_smoker + has_chip + chip_af')
				fit = tryCatch(
				...
				tidy(fit) |>
					filter(str_detect(term, '(chip)|(dnmt3a)|(tet2)|(asxl1)')) |>
